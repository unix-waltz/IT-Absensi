...
<form action="" method="post">
    <?php while ($row = mysqli_fetch_object($query)) : ?>
      ...
      <td ><?=$row->divisi ?></td>
            ...
            <td>
            <div class="form-check form-check-inline">
                    <input class="form-check-input" type="checkbox" name="kehadiran[<?= $row->id ?>]" id="inlineRadio3_<?= $row->id ?>" value="SP" <?= ($row->absen === 'SP') ? 'checked' : '' ?>>
                    <label class="form-check-label" for="inlineRadio3_<?= $row->id ?>"></label>
                    (&nbsp;<?= $row->sp ?>&nbsp;)
                </div>
